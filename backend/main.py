...
# S3 Configuration
session = get_session()
s3_client = session.create_client('s3', region_name=AWS_REGION, aws_access_key_id=AWS_ACCESS_KEY_ID, aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
...
async def s3_upload(contents: bytes, key: str):
    ...
    try:
        await s3_client.put_object(Bucket=AWS_BUCKET, Key=key, Body=contents)
...
async def s3_download(key: str):
    try:
        response = await s3_client.get_object(Bucket=AWS_BUCKET, Key=key)
